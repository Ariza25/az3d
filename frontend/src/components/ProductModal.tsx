...
import { Check, Heart, Layers, Minus, Play, Plus, ShoppingBag, Star, X, FileText, MessageSquare, Image as ImageIcon, Send, ShieldCheck, User, Tag, Sparkles } from 'lucide-react';
...
export const ProductModal: React.FC<ProductModalProps> = ({ product, onClose }) => {
  ...
  const couponShippingDiscount = (appliedCoupon?.applies_to_shipping && freightPrice > 0)
          ...
          <div className="flex min-h-0 flex-col bg-chumbo-900 p-5 sm:p-8 lg:overflow-y-auto lg:p-10">
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-chumbo-800 bg-chumbo-950/60 p-4 sm:p-5 space-y-5">
                ...
                {/* Desconto Progressivo de Atacado */}
                <div className="rounded-2xl border border-laser-400/20 bg-laser-400/5 p-3.5">
                  ...
                  <div className="mt-2.5 grid grid-cols-3 gap-2 text-center text-[11px]">
                    <div className={`rounded-xl border p-2 transition ${quantity >= 3 && quantity <= 5 ? 'border-emerald-400 bg-emerald-500/20 font-bold text-emerald-300 shadow-sm' : 'border-chumbo-800 bg-chumbo-900/60 text-slate-400'}`}>
                      <span className="block text-[10px]">3 a 5 un</span>
                      <span className="text-xs font-extrabold text-white">2% OFF</span>
                    </div>
                    <div className={`rounded-xl border p-2 transition ${quantity >= 6 && quantity <= 9 ? 'border-emerald-400 bg-emerald-500/20 font-bold text-emerald-300 shadow-sm' : 'border-chumbo-800 bg-chumbo-900/60 text-slate-400'}`}>
                      <span className="block text-[10px]">6 a 9 un</span>
                      <span className="text-xs font-extrabold text-white">5% OFF</span>
                    </div>
                    <div className={`rounded-xl border p-2 transition ${quantity >= 10 ? 'border-emerald-400 bg-emerald-500/20 font-bold text-emerald-300 shadow-sm' : 'border-chumbo-800 bg-chumbo-900/60 text-slate-400'}`}>
                ...
                <div className="border-t border-chumbo-800 pt-3">
                  ...
                  {couponMessage && (
                    <p className={`mt-1.5 text-[11px] ${couponMessage.type === 'error' ? 'text-rose-400' : 'text-emerald-400'}`}>
                      {couponMessage.text}
                    </p>
                  )}
                </div>
                      ...
                      <div className="mt-1 flex flex-col gap-0.5 text-[11px] text-slate-400">
                        {wholesale.percent > 0 ? (
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            <span className="rounded bg-emerald-500/20 px-1 py-0.2 text-[10px] font-bold text-emerald-400">-{wholesale.percent}%</span>
                          </div>
                        ) : (
                        ...
                        {selectedFreight && (
                          <span className="font-mono text-laser-400 font-bold">
                            + Frete ({selectedFreight.name}): {couponShippingDiscount > 0 ? (
                              <>
