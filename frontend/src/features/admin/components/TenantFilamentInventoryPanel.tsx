    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-chumbo-800 dark:bg-chumbo-900/60">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-extrabold text-slate-900 dark:text-white">Estoque de Insumos & Filamentos 3D</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400">Gerencie carretéis de PLA, ABS, PETG, TPU e galões de Resina</p>
          </div>
        </div>
      ...
      {isAddOpen && (
        <form onSubmit={handleAddSpool} className="rounded-2xl border border-cyan-200 bg-white p-5 shadow-sm space-y-4 dark:border-laser-500/30 dark:bg-chumbo-900">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div>
              <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Nome do Insumo</label>
              <input
                type="text"
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-slate-500 shadow-sm dark:border-chumbo-800 dark:bg-chumbo-900/40 dark:text-slate-400">
          ...
          <p className="font-semibold text-slate-900 dark:text-white">Nenhum insumo cadastrado no banco</p>
        ...
        {spools.map((spool) => {
          ...
          return (
            <div key={spool.id} className="relative rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-3 dark:border-chumbo-800 dark:bg-chumbo-900">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <span className="h-4 w-4 rounded-full border shadow-sm" style={{ backgroundColor: spool.color_hex }} />
                  <div>
                    <strong className="block text-sm text-slate-900 dark:text-white">{spool.name}</strong>
                    <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">{spool.material_type} · {spool.color_name}</span>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className={`font-mono font-bold ${isLow ? 'text-rose-700 dark:text-rose-400' : 'text-emerald-700 dark:text-emerald-400'}`}>
                ...
                <div className="h-2 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-chumbo-950">
                  <div
                    className={`h-full transition-all ${isLow ? 'bg-rose-600' : 'bg-cyan-600 dark:bg-laser-400'}`}
              ...
              {isLow && (
                <div className="flex items-center gap-1.5 text-[11px] font-bold text-amber-800 bg-amber-50 border border-amber-300 p-2 rounded-lg dark:text-amber-400 dark:bg-amber-500/10 dark:border-amber-500/20">
                  <AlertTriangle className="h-3.5 w-3.5" />
                  <span>Estoque de filamento baixo!</span>
                </div>
              )}

              <div className="flex items-center justify-between border-t border-slate-200 pt-2 text-xs dark:border-chumbo-800/80">
                <span className="text-slate-500 dark:text-slate-400">Preço Custo/kg:</span>
                <span className="font-mono font-bold text-slate-900 dark:text-white">{money(spool.price_per_kg)}</span>
              </div>
