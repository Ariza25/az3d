...
func main() {
	...
	if err := run(ctx, os.Args[1:], os.Stdin, os.Stdout, os.Stderr); err != nil {
		fmt.Fprintln(os.Stderr, "az3d-amazon-seller-mcp:", err)
		os.Exit(1)
	}
}
...
func run(ctx context.Context, args []string, _ io.Reader, output, errorOutput io.Writer) error {
	...
	case "serve", "doctor":
		...
		tenantID := flags.Uint("tenant-id", 0, "tenant com conta Amazon Seller autorizada no painel AZ3D")
		if err := flags.Parse(commandArgs); err != nil {
			return err
		}
		if *tenantID == 0 {
			return errors.New("informe --tenant-id com o tenant que autorizou a conta Amazon Seller")
		}
		cfg := config.LoadConfig()
		if cfg.DatabaseURL == "" && !cfg.DatabaseRequired {
			return errors.New("o MCP requer o banco da plataforma; configure DATABASE_URL")
		}
		connector := amazon.New()
		db, err := database.OpenExistingDB(cfg)
		if err != nil {
			return fmt.Errorf("nao foi possivel conectar ao banco da plataforma: %w", err)
		}
		accounts := mcpserver.NewAmazonDatabaseAccountSource(db, connector, *tenantID, cfg.CredentialEncryptionKey)
		if command == "doctor" {
			result, err := mcpserver.RunDoctor(ctx, connector, accounts)
			if err != nil {
				return fmt.Errorf("diagnostico falhou: %w", err)
			}
			fmt.Fprintf(output, "OK: Amazon Seller conectada; tenant=%d seller=%s marketplace=%s; marketplaceParticipations respondeu com sucesso.\n", *tenantID, result.SellerID, result.Marketplace)
			return nil
		}
		server := mcpserver.New(connector, accounts).MCPServer()
		logger := log.New(errorOutput, "az3d-amazon-seller-mcp: ", log.LstdFlags)
		if err := server.Run(ctx, &mcp.StdioTransport{}); err != nil {
			logger.Printf("servidor finalizado: %v", err)
		}
		return nil
	case "help", "-h", "--help":
		fmt.Fprintln(output, "Uso: az3d-amazon-seller-mcp [serve|doctor] --tenant-id ID")
		return nil
	default:
