...
func TestHelpDocumentsShopeeTenantSelection(t *testing.T) {
	var output bytes.Buffer
	if err := run(context.Background(), []string{"help"}, strings.NewReader(""), &output, &output); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(output.String(), "az3d-shopee-seller-mcp") || !strings.Contains(output.String(), "--tenant-id") {
		t.Fatalf("help must document Shopee tenant selection: %q", output.String())
	}
}
